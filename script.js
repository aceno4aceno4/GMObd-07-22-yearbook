...
mapClass:
    ...
    defaultMap ||
    {
       ...
       shortMessage:
    row["Напишите короткое пожелание или сообщение для своих одногруппников. Можно использовать любой язык."] || "",

        explanation:
            row["Если вы хотите рассказать историю или значение вашего сообщения, можете написать об этом на русском или английском языке (Необязательно)"] || "",

        publicMessage:
            row["Чем бы вы хотели поделиться со своими одногруппниками? (What would you like to share with your classmates?)"] || "",

        privateMessage:
            row["Спасибо, что нашли время заполнить анкету. Если вы хотите оставить личное сообщение автору выпускного альбома, можете написать его здесь. Это сообщение не будет опубликовано на сайте."] || "",
